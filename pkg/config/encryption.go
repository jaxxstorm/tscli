...
func validateAgeEncryptionConfig(cfg AgeEncryptionConfig) error {
	...
	if !hasPublicKey && !hasConfigPrivateKey && !hasCommand {
		return nil
	}
	if !hasPublicKey {
		return fmt.Errorf("encryption.age.public-key is required when config encryption is enabled")
	}
	if hasConfigPrivateKey && hasCommand {
		return fmt.Errorf("configure either encryption.age.private-key or encryption.age.private-key-command, not both")
	}
...
func resolveAgeIdentity(v *viper.Viper) (age.Identity, error) {
	...
	return nil, fmt.Errorf("encrypted config requires TSCLI_AGE_PRIVATE_KEY, encryption.age.private-key-command, or encryption.age.private-key")
}
...
func SetAgeEncryptionConfig(cfg AgeEncryptionConfig) error {
	...
	if cfg.PublicKey == "" && cfg.PrivateKey == "" && cfg.PrivateKeyCommand == "" {
