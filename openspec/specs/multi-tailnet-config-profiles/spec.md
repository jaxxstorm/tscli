...
- **WHEN** a profile contains neither `api-key` nor a complete `oauth-client-id` and `oauth-client-secret` pair
...
For operational commands, effective values SHALL be resolved in this order: flags, environment variables, active profile, legacy flat config keys where supported by the command. API-key commands SHALL continue to resolve `api-key` and `tailnet` using that precedence. OAuth lifecycle commands SHALL resolve `oauth-client-id` and `oauth-client-secret` using the same precedence model, then exchange those credentials for a bearer token at runtime.
...
#### Scenario: Active OAuth profile is used by default
- **WHEN** OAuth flags and environment variables are absent and `active-tailnet` maps to a profile with `oauth-client-id` and `oauth-client-secret`
- **THEN** OAuth lifecycle commands SHALL use that profile for token exchange
...
The project SHALL include unit and integration tests for profile parsing, validation, runtime resolution, and profile command behavior across both API-key-backed and OAuth-backed profiles.
...
#### Scenario: Integration tests cover OAuth profile command flows
- **WHEN** command-level integration tests execute for config profile operations and OAuth-backed lifecycle commands
- **THEN** tests SHALL verify validation failures, persisted state transitions, auth resolution, and successful lifecycle requests using active profiles
