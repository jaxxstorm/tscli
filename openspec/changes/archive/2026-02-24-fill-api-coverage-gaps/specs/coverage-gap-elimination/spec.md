...
Coverage-gap reporting SHALL enforce zero missing in-scope OpenAPI operations and zero unknown mapped commands before the change is considered complete.
...
Operations intentionally not implemented SHALL be declared in an exclusions policy with rationale.
...
CI SHALL run parity checks and fail when new uncovered operations or unmapped command regressions are introduced.

#### Scenario: Regression introduced in pull request
- **WHEN** a pull request increases uncovered in-scope operations or unmapped commands relative to baseline
